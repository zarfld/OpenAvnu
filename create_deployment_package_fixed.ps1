...
# Milan configuration
$milanConfigPath = Join-Path $OutputPath "config/milan_professional_audio.conf"
$milanConfig = @"
...
$milanConfig | Out-File -FilePath $milanConfigPath -Encoding UTF8
Write-Success "Created Milan configuration file"

# Network configuration
$networkConfigPath = Join-Path $OutputPath "config/network_professional.conf"
$networkConfig = @"
...
$deploymentScriptPath = Join-Path $OutputPath "scripts/deploy_milan_system.ps1"
$deploymentScript = @'
#!/usr/bin/env powershell
