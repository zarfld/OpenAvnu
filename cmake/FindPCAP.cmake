...
if(WIN32)
  ...
  if(DEFINED ENV{NPCAP_SDK_DIR})
    ...
    message(STATUS "[OK] Found Npcap SDK (CI): ${PCAP_ROOT_DIR}")
  elseif(DEFINED ENV{NPCAP_DIR})
    ...
    add_definitions(-DUSING_NPCAP)
    message(STATUS "[OK] Found Npcap SDK: ${PCAP_ROOT_DIR}")
  elseif(DEFINED ENV{WPCAP_DIR})
    ...
    foreach(NPCAP_PATH ${NPCAP_STANDARD_PATHS})
      if(EXISTS "${NPCAP_PATH}/Include" AND EXISTS "${NPCAP_PATH}/Lib")
        ...
        add_definitions(-DUSING_NPCAP)
        message(STATUS "[OK] Found Npcap SDK at standard location: ${PCAP_ROOT_DIR}")
        break()
      endif()
    endforeach()
  ...
  foreach(INC_DIR ${PCAP_POSSIBLE_INCLUDE_DIRS})
    if(EXISTS "${INC_DIR}")
      ...
      if(EXISTS "${INC_DIR}/pcap.h" OR EXISTS "${INC_DIR}/pcap/pcap.h")
        ...
        message(STATUS "[DIR] Using PCAP include directory: ${PCAP_INCLUDE_DIRS}")
        break()
      endif()
    endif()
  endforeach()
  ...
  foreach(LIB_DIR ${PCAP_POSSIBLE_LIB_DIRS})
    if(EXISTS "${LIB_DIR}")
      ...
      message(STATUS "[DIR] Using PCAP library directory: ${PCAP_LIBRARY_DIRS}")
      break()
    endif()
  endforeach()
  ...
  if(NOT PCAP_LIBRARY_DIRS)
    ...
    if(PCAP_LIB_FILES)
      ...
      message(STATUS "[DIR] Using fallback PCAP library directory: ${PCAP_LIBRARY_DIRS}")
    else()
      message(WARNING 
  ...
  foreach(LIB_NAME ${PCAP_POSSIBLE_LIBRARY_NAMES})
    ...
    if(PCAP_LIBRARY_${LIB_NAME})
      ...
      message(STATUS "[LIB] Found PCAP library: ${PCAP_LIBRARIES}")
      break()
    endif()
  endforeach()
  ...
  if(PKG_CONFIG_FOUND)
    ...
    if(PCAP_FOUND)
      ...
      message(STATUS "[OK] Found libpcap via pkg-config: ${PCAP_LIBRARIES}")
    endif()
  endif()
  ...
  if(NOT PCAP_FOUND)
    ...
    if(PCAP_LIBRARIES AND PCAP_INCLUDE_DIRS)
      ...
      message(STATUS "[OK] Found libpcap: ${PCAP_LIBRARIES}")
    endif()
  endif()
...
if(PCAP_LIBRARIES AND PCAP_INCLUDE_DIRS)
  set(PCAP_FOUND TRUE)
  message(STATUS "[PKG] PCAP Summary:")
