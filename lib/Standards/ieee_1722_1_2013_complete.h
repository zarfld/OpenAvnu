/**
 * @file ieee_1722_1_2013_complete.h
 ...
 * 
 * This header provides complete structures and definitions for:
 * - AVDECC Discovery Protocol (ADP) - IEEE 1722.1-2013 Clause 6
 * - AVDECC Entity Model (AEM) - IEEE 1722.1-2013 Clause 7  
 * - AVDECC Connection Management Protocol (ACMP) - IEEE 1722.1-2013 Clause 8
 * - AVDECC Enumeration and Control Protocol (AECP) - IEEE 1722.1-2013 Clause 9
 * 
 * @version 1.0.0
 * @date Juli 2025
...
// Cross-platform networking headers
    ...
    #include <winsock2.h>
    #include <ws2tcpip.h>
    // Windows network byte order functions
    #define htonll(x) ((((uint64_t)htonl(x)) << 32) + htonl((x) >> 32))
    #define ntohll(x) ((((uint64_t)ntohl(x)) << 32) + ntohl((x) >> 32))
#else
    #include <arpa/inet.h>
    #include <netinet/in.h>
    // Linux/Unix network byte order functions  
...
namespace _2013 {
    ...
    // ==============================
    // ADP (AVDECC Discovery Protocol) - IEEE 1722.1-2013 Clause 6
    // ==============================
    ...
    namespace ADP {
        ...
        public:
            // ADP fields as per IEEE 1722.1-2013 Table 6.6
            ADPMessageType message_type;             ///< Message type
            uint8_t valid_time;                      ///< Valid time in 2-second increments
        ...
        public:
            ...
            // Setters for easy configuration
    ...
    // ==============================
    // AEM (AVDECC Entity Model) - IEEE 1722.1-2013 Clause 7
    // ==============================

    namespace AEM {

        /**
         * @brief Descriptor Types (Table 7.1)
         */
        ...
        /**
         * @brief ENTITY Descriptor (Table 7.2)
         */
        ...
        /**
         * @brief Stream Formats (Table 7.8)
         */
        enum StreamFormat : uint64_t {
            IEC_61883_IIDC_BASE             = 0x00A0020140000000ULL,
            IEC_61883_AM824_BASE            = 0x00A0020240000000ULL,
            IEC_61883_AM824_MBLA_96KHZ_8CH  = 0x00A002024001080AULL,
            IEC_61883_AM824_MBLA_48KHZ_8CH  = 0x00A0020240010408ULL,
            AAF_PCM_48KHZ_16BIT_2CH         = 0x02A0040240020002ULL,
            AAF_PCM_48KHZ_24BIT_2CH         = 0x02A0060240020002ULL,
            AAF_PCM_96KHZ_24BIT_2CH         = 0x02A0060260020002ULL,
            MJPEG                           = 0x80000000000000FCULL,
            H264                            = 0x8000000000000096ULL
        };
        ...
        /**
         * @brief Utility function for safe string copying
         */
    ...
    // ==============================
    // AECP (Enumeration and Control Protocol) - IEEE 1722.1-2013 Clause 9
    // ==============================

    namespace AECP {

        /**
         * @brief AECP Message Types (Table 9.1)
         */
        ...
        /**
         * @brief AEM Command Types (Table 9.2)
         */
        ...
        /**
         * @brief AEM Status Codes (Table 9.3)
         */
    ...
    // ==============================
    // ACMP (Connection Management Protocol) - IEEE 1722.1-2013 Clause 8
    // ==============================

    namespace ACMP {

        /**
         * @brief ACMP Message Types (Table 8.1)
         */
        ...
        /**
         * @brief ACMP Status Codes (Table 8.2)
         */
        ...
        /**
         * @brief ACMP Connection Flags
         */
        enum ACMPFlags : uint16_t {
            CLASS_A                = 0x0000, ///< Class A stream (default)
            CLASS_B                = 0x0001, ///< Class B stream
            FAST_CONNECT           = 0x0002,
            SAVED_STATE            = 0x0004,
        ...
        /**
         * @brief ACMP Message structure (Table 8.3)
         */
        struct ACMPMessage {
            ACMPMessageType message_type;           ///< Message type
            ACMPStatus status;                      ///< Status field  
            uint16_t control_data_length;           ///< Length of control_data
            ...
            ACMPFlags flags;                        ///< Connection flags
    ...
    // ==============================
    // Utility Functions and Common Structures
    // ==============================

    /**
     * @brief IEEE 1722 AVTPDU Common Header for AVDECC (first 12 bytes)
     */
    struct AVTPDUHeader {
        uint8_t subtype;                    ///< AVTP subtype (0xFA=ADP, 0xFB=AECP, 0xFC=ACMP)
        uint8_t sv_ver_mr_gv_tv_seq_tu;     ///< Various flags and sequence number
        uint16_t control_data_length;       ///< Length of AVDECC message data
        uint64_t target_entity_id;          ///< Target Entity ID (or stream_id for ADP)

        // Methods
        AVTPDUHeader();
        void set_subtype(uint8_t subtype) { this->subtype = subtype; }
        void set_control_data_length(uint16_t length) { control_data_length = htons(length); }
        void set_target_entity_id(uint64_t entity_id) { target_entity_id = htonll(entity_id); }
        
        uint8_t get_subtype() const { return subtype; }
        uint16_t get_control_data_length() const { return ntohs(control_data_length); }
        uint64_t get_target_entity_id() const { return ntohll(target_entity_id); }
    };

    /**
     * @brief Complete AVDECC entity representation for IEEE 1722.1-2013
     */
    class AVDECCEntity_2013 {
    public:
        // Entity information
        uint64_t entity_id;
        uint64_t entity_model_id;
        ADP::EntityCapabilities entity_capabilities;
        
        // Stream capabilities
        uint16_t talker_stream_sources;
        ADP::TalkerCapabilities talker_capabilities;
        uint16_t listener_stream_sinks;
        ADP::ListenerCapabilities listener_capabilities;
        ADP::ControllerCapabilities controller_capabilities;
        ...
        // Methods
        AVDECCEntity_2013();
        ADP::AVDECCDiscoveryProtocolPDU create_adp_message(ADP::ADPMessageType type) const;
        bool update_from_adp(const ADP::AVDECCDiscoveryProtocolPDU& adpdu);
        AEM::EntityDescriptor get_entity_descriptor() const;
        
        // Validation
        bool is_valid() const;
        std::string get_capabilities_string() const;
    };
