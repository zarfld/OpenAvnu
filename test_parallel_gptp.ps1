...
if (-not $i210) {
    Write-Host "ERROR: I210 adapter not found or not connected" -ForegroundColor Red
    exit 1
}
...
if (-not (Test-Path $gptpExe)) {
    Write-Host "ERROR: gPTP executable not found: $gptpExe" -ForegroundColor Red
    exit 1
}
...
$i210Process = Start-Process -FilePath $gptpExe -ArgumentList @($i210.MacAddress) -RedirectStandardOutput "gptp_i210_${timestamp}.log" -RedirectStandardError "gptp_i210_${timestamp}.err" -PassThru -NoNewWindow

Start-Sleep -Seconds 2

# Start I226 instance  
Write-Host "Starting I226 gPTP daemon..." -ForegroundColor Cyan
$i226Process = Start-Process -FilePath $gptpExe -ArgumentList @($i226.MacAddress) -RedirectStandardOutput "gptp_i226_${timestamp}.log" -RedirectStandardError "gptp_i226_${timestamp}.err" -PassThru -NoNewWindow

Write-Host ""
Write-Host "Running parallel test for $Duration seconds..." -ForegroundColor Yellow
Write-Host ""
...
while ((Get-Date) -lt $endTime) {
    ...
    if (-not $i210Running -and -not $i226Running) {
        Write-Host "Both processes stopped early, ending test..." -ForegroundColor Yellow
        break
    }
...
if ($i210Started -and $i226Started) {
    ...
    Write-Host "PARALLEL TEST: PARTIAL SUCCESS" -ForegroundColor Yellow -BackgroundColor Black
